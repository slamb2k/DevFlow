...
export class RateLimiter extends EventEmitter {
  ...
  async check_limit(service) {
    ...
    // Check limits
    if (config.requests_per_second && window.requests.filter(t => now - t < 1000).length >= config.requests_per_second) {
      metrics.blocked++
      return { allowed: false, retry_after: 1 }
    }

    if (config.requests_per_minute && window.requests.filter(t => now - t < 60000).length >= config.requests_per_minute) {
      metrics.blocked++
      return { allowed: false, retry_after: 60 }
    }

    if (config.requests_per_hour && window.requests.filter(t => now - t < 3600000).length >= config.requests_per_hour) {
      metrics.blocked++
  ...
  update_from_headers(platform, headers) {
    ...
    this.rate_limit_status.set(platform, {
      ...
      remaining: parseInt(normalized['x-ratelimit-remaining'] || normalized['ratelimit-remaining'] || '0'),
   ...
   */
  get_status(platform) {
    return this.rate_limit_status?.get(platform) || {
      limit: 0,
      remaining: 0,
      reset: 0,
      retry_after: 0,
  ...
  record_response_time(service, time) {
    ...
    if (adaptive) {
      ...
      if (avg_time > adaptive.target_response_time) {
        // Slow down
        adaptive.current_rate = Math.max(
          adaptive.min_rate,
          adaptive.current_rate * 0.9
        )
      } else {
        // Speed up
        adaptive.current_rate = Math.min(
          adaptive.max_rate,
          adaptive.current_rate * 1.1
        )
      }
    }
  }
