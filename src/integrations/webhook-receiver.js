...
export class WebhookReceiver {
  ...
  async process_webhook(platform, req, res) {
    ...
    switch (platform) {
      ...
      case 'slack':
        ...
        if (event_data === null) return // Already handled
        break
      default:
  ...
  validate_github_signature(req, secret) {
    ...
    const expected = this.generate_github_signature(
  ...
  validate_slack_signature(req, secret) {
    ...
    const expected = this.generate_slack_signature(
      JSON.stringify(req.body),
      secret,
      timestamp
    )
  ...
  async reprocess_failed_webhooks() {
    ...
    for (const webhook of failed) {
      try {
        await this.event_bus.emit_async(
          `${webhook.platform}:reprocessed`,
          webhook.payload
        )
        processed++
      } catch (error) {
