...
describe('CredentialManager', () => {
  ...
  describe('Credential Storage', () => {
    ...
    it('should retrieve and decrypt credentials', async () => {
      ...
      const encrypted_data = credential_manager.encrypt(
        JSON.stringify(original_credentials)
      )
      ...
      mockFs.readFile.mockImplementation((path) => {
        if (path.includes('github')) {
          return Promise.resolve(
            credential_manager.encrypt(
              JSON.stringify({ platform: 'github', token: 'token1' })
            )
          )
        }
        if (path.includes('gitlab')) {
          return Promise.resolve(
            credential_manager.encrypt(
              JSON.stringify({ platform: 'gitlab', token: 'token2' })
            )
          )
        }
  ...
  describe('Token Validation', () => {
    it('should validate token expiration', async () => {
      ...
      mockFs.readFile.mockResolvedValue(
        credential_manager.encrypt(JSON.stringify(expired_token))
      )
    ...
    it('should validate non-expired tokens', async () => {
      ...
      mockFs.readFile.mockResolvedValue(
        credential_manager.encrypt(JSON.stringify(valid_token))
      )
  ...
  describe('OAuth Token Management', () => {
    ...
    it('should auto-refresh expired tokens on retrieval', async () => {
      ...
      mockFs.readFile.mockResolvedValue(
        credential_manager.encrypt(JSON.stringify(expired_oauth))
      )
    ...
    it('should handle refresh failures', async () => {
      ...
      const mock_refresh_handler = jest
        .fn()
      ...
      mockFs.readFile.mockImplementation((path) => {
        ...
        for (const [key, value] of Object.entries(accounts)) {
          if (path.includes(key.replace(':', '_'))) {
            return Promise.resolve(
              credential_manager.encrypt(JSON.stringify(value))
            )
          }
        }
        return Promise.reject(new Error('Not found'))
  ...
  describe('Credential Backup and Recovery', () => {
    ...
    it('should import credentials from backup', async () => {
      ...
      const mock_decrypt = jest
        .spyOn(credential_manager, 'decrypt_backup')
        .mockReturnValue({
      ...
      expect(mock_decrypt).toHaveBeenCalledWith(
